...
func NewChatExtractor(dataPath, selfWxId, targetWxId string) (*ChatExtractor, error) {
	extractor := &ChatExtractor{
		...
		ImageOutputPath: filepath.Join(dataPath, "FileStorage", "Image"),
	}
...
func (ce *ChatExtractor) GetMessageText(msg WeChatMessage) string {
	...
	case Wechat_Message_Type_Picture:
		...
		if msg.ThumbPath != "" {
			return fmt.Sprintf("[图片] %s", msg.ThumbPath)
		}
		// 尝试查找图片文件
...
func (ce *ChatExtractor) ExtractChatHistory() ([]ChatSession, error) {
	...
	// 处理图片文件转换
	fmt.Println("正在处理图片文件转换...")
	messages, err = ce.processImageFiles(messages)
	if err != nil {
		log.Printf("处理图片文件时出现错误: %v", err)
	}
...
// 用户选择聊天对象
func selectContact(contacts []Contact) (Contact, error) {
	// 按消息数量排序
	...
	if err != nil || index < 1 || index > len(contacts) {
		return Contact{}, fmt.Errorf("无效的序号: %s", input)
	}

	return contacts[index-1], nil
}
...
func main() {
	...
	// 用户选择聊天对象
	selectedContact, err := selectContact(contacts)
	if err != nil {
	...
	fmt.Printf("目标用户微信ID: %s\n", selectedContact.UserName)
	fmt.Printf("正在提取与 %s 的聊天记录...\n", selectedContact.NickName)
	...
	// 获取用户昵称
	selfNickName, err := extractor.GetUserInfo(extractor.SelfWxId)
	if err != nil {
		log.Printf("获取自己的昵称失败，使用微信ID: %s", extractor.SelfWxId)
		selfNickName = extractor.SelfWxId
	}
...
// 生成唯一的文件名
func (ce *ChatExtractor) generateUniqueFileName(originalPath, msgSvrId string) string {
	// 使用原始路径和消息ID生成MD5哈希，确保唯一性
	hash := md5.Sum([]byte(originalPath + msgSvrId + time.Now().Format("2006-01-02 15:04:05")))
	hashStr := hex.EncodeToString(hash[:])[:12] // 取前12位作为文件名
	...
	case ".bmp":
		outputExt = ".bmp"
	default:
		outputExt = ".jpg" // 默认为jpg格式
	}
	
	return fmt.Sprintf("img_%s%s", hashStr, outputExt)
}

// 转换.dat文件为图片格式
func (ce *ChatExtractor) convertDatToImage(datPath, msgSvrId string) (string, error) {
	// 确保输出目录存在
	if err := os.MkdirAll(ce.ImageOutputPath, 0755); err != nil {
		return "", fmt.Errorf("创建图片输出目录失败: %v", err)
	}
	
	// 生成唯一的文件名
	fileName := ce.generateUniqueFileName(datPath, msgSvrId)
	outputPath := filepath.Join(ce.ImageOutputPath, fileName)
	...
	for {
		...
		outputPath = filepath.Join(ce.ImageOutputPath, fmt.Sprintf("%s_%d%s", baseName, counter, ext))
		counter++
	}
	
	// 使用wechat包的DecryptDat函数解码.dat文件
	err := wechat.DecryptDat(datPath, outputPath)
	if err != nil {
		return "", fmt.Errorf("解码.dat文件失败: %v", err)
	}
	
	log.Printf("成功转换图片: %s -> %s", datPath, outputPath)
	return outputPath, nil
}

// 处理消息中的图片文件
func (ce *ChatExtractor) processImageFiles(messages []WeChatMessage) ([]WeChatMessage, error) {
	for i := range messages {
		msg := &messages[i]
		
		// 只处理图片消息的主要图片，不处理缩略图
		if msg.Type == Wechat_Message_Type_Picture {
			// 只处理ImagePath，不处理ThumbPath
			if msg.ImagePath != "" && strings.HasSuffix(msg.ImagePath, ".dat") {
				if _, err := os.Stat(msg.ImagePath); err == nil {
					newPath, err := ce.convertDatToImage(msg.ImagePath, msg.MsgSvrId)
					if err != nil {
						log.Printf("转换图片失败: %v", err)
					} else {
						msg.ImagePath = newPath
					}
				}
			}
		}
		
		// 不处理视频消息的缩略图和其他类型的缩略图
	}
